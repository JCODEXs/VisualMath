...
export const BOX_PRESETS = {
  S: { W: 10, D: 10, H: 20, label: "10 × 10 × 20" },
  L: { W: 30, D: 30, H: 50, label: "30 × 30 × 50" },
};

const CUBE_SIZE = 0.82; // < 1.0 → visible rivet groove between cubes
const MAX_PIECE_CELLS = 64;
const VERT_THRESH_RATIO = 0.55; // vertical column clears below H × this
const GHOST_OPACITY = 0.16;

// ms between automatic drops, indexed by level (clamped at level 10)
const DROP_SPEEDS = [900, 720, 570, 450, 350, 270, 200, 145, 100, 70];
...
const PIECE_COLORS = [
  new THREE.Color("#ef4444"), // rojo
  new THREE.Color("#3b82f6"), // azul
  new THREE.Color("#22c55e"), // verde
  new THREE.Color("#f59e0b"), // naranja
  new THREE.Color("#a855f7"), // violeta
];
...
function checkAndClear(grid, W, D, H) {
  ...
  if (W === D) {
    for (let ky = 0; ky < H; ky++) {
      ...
      let antiFull = true;
      for (let i = 0; i < W; i++) {
        if (!g.filled[gIdx(i, W - 1 - i, ky, W, D)]) {
          antiFull = false;
          break;
        }
      }
...
const TetrisScene = forwardRef(function TetrisScene({ preset }, ref) {
          ...
          for (let z = 0; z < D; z++)
            for (let y = 0; y < H; y++) {
              ...
              if (grid.filled[i]) {
                ...
                items.push({ wx, wy, wz, hex: grid.colors[i] || "#8912c4ff" });
              }
            }
  ...
  useEffect(() => {
    ...
    scene.add(sun);
    // Luces de relleno (descomentadas y ajustadas)
    const ambient = new THREE.DirectionalLight(0xffffff, 0.6);
    ambient.position.set(10, 20, -10);
    ambient.target.position.set(0, 2, 0);
    scene.add(ambient);
    ...
    dir2.position.set(0, 20, -5);
    dir2.target.position.set(0, 10, 0);
    scene.add(dir2);
    const rim = new THREE.DirectionalLight(0xffffff, 1);
    rim.position.set(-10, 20, 0);
    rim.target.position.set(0, 10, 0);
    scene.add(rim);
    const rim2 = new THREE.DirectionalLight(0xffffff, 1);
    rim2.position.set(-10, 12, 10);
    rim2.target.position.set(0, 15, 0);
    scene.add(rim2);
    const rim3 = new THREE.DirectionalLight(0xffffff, 1);
    rim3.position.set(-10, 10, -10);
    rim3.target.position.set(0, 8, 0);
    scene.add(rim3);

    // const helper = new THREE.DirectionalLightHelper(dir, 2);
    // scene.add(helper);
    // const helper2 = new THREE.DirectionalLightHelper(dir2, 2);
    // scene.add(helper2);
    // const helper3 = new THREE.DirectionalLightHelper(rim, 2);
    // scene.add(helper3);
    // const helper4 = new THREE.DirectionalLightHelper(rim2, 2);
    // scene.add(helper4);
    // const helper5 = new THREE.DirectionalLightHelper(rim3, 2);
    // scene.add(helper5);
    // const helper6 = new THREE.DirectionalLightHelper(ambient, 2);
    // scene.add(helper6);
    // const helper7 = new THREE.DirectionalLightHelper(sun, 2);
    // scene.add(helper7);
...
function rebuildInstancedMeshes(t, W, D, H) {
  ...
  ["gridMesh", "pieceMesh", "ghostMesh"].forEach((key) => {
    if (t[key]) scene.remove(t[key]);
  });

  const geo = new THREE.BoxGeometry(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE);

  // Grid cells — max W * D * H
  ...
  t.gridMesh = createInstancedMesh(geo, gridMat, W * D * H); //t.gridMesh = new THREE.InstancedMesh(geo, gridMat, W * D * H);
  t.gridMesh.castShadow = t.gridMesh.receiveShadow = true;
  t.gridMesh.count = 0;
  scene.add(t.gridMesh);

  // Active piece — max MAX_PIECE_CELLS
  ...
  t.pieceMesh = createInstancedMesh(geo, pieceMat, MAX_PIECE_CELLS); //t.pieceMesh = new THREE.InstancedMesh(geo, pieceMat, MAX_PIECE_CELLS);
  t.pieceMesh.castShadow = true;
  t.pieceMesh.count = 0;
  scene.add(t.pieceMesh);

  // Ghost piece — transparent, same geo
  ...
  t.ghostMesh = new THREE.InstancedMesh(geo, ghostMat, MAX_PIECE_CELLS);
  t.ghostMesh.count = 0;
